...
import biz.advanceitgroup.taxirideserver.commons.entities.Mail;
...
public class MailService {
	 ...
	 * status
	 */
	public void sendEmailNotification(String message, String to)
			throws IOException, TemplateException, MessagingException {
		Mail mail = new Mail();
		mail.setSubject("Taxiride Email notification");
		mail.setTo(to);
		mail.setFrom(mailFrom);
		mail.getModel().put("userName", to);
		mail.getModel().put("action", message);
		mail.getModel().put("actionStatus", message);

		Template template = templateConfiguration.getTemplate("email-notification.ftl");
		String mailContent = FreeMarkerTemplateUtils.processTemplateIntoString(template, mail.getModel());
		mail.setContent(mailContent);
		send(mail);
	}
