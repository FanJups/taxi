...
public class JwtTokenProvider {
	...
	//Génerate token for Social user
	public String generateTokenForSocialEmail(socialRegistrationRequest registrationRequest) {
		Instant expiryDate = Instant.now().plusMillis(jwtExpirationInMs);
		return Jwts.builder()
				.setSubject(registrationRequest.getPhoneNumber())
				.setIssuedAt(Date.from(Instant.now()))
				.setExpiration(Date.from(expiryDate))
				.signWith(SignatureAlgorithm.HS512, jwtSecret)
				.compact();
	}
	//Générer un token pour le numéro de téléphone
	public String generateTokenFromUserPhone(String phone) {
		Instant expiryDate = Instant.now().plusMillis(jwtExpirationInMs);
		return Jwts.builder()
				.setSubject(phone)
				.setIssuedAt(Date.from(Instant.now()))
